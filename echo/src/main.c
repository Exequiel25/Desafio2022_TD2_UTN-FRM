...
int main( void ) {
  ...
  // Enable peripheral clocks and set up GPIO pins.
    ...
    // Configure pin A2(Tx) A3(Rx) for USART2.
  ...
  // Set the baud rate to 12000.
  uint16_t uartdiv = SystemCoreClock / 12000;
  #ifdef VVC_F1
  ...
  // Main loop: transmit converted value from ADC in serial virtual terminal.
  char rxb = '\0';
  while ( 1 ) {
    // ADC read value
    #ifdef VVC_F1
    ...
    // Transmit the byte of data once the peripheral is ready.
    #ifdef VVC_F1
